...
export class BaseStoreRepository<T = any> {
  ...
  constructor(state$: Observable<ExtendedElfStore<T>>, store: ElfBaseStore<ExtendedElfStore<T>>, initialState: T) {
    this.state$ = state$.pipe(select(state => state));
    this.store = store;
    this.initialState = JSON.parse(JSON.stringify({ ...initialState, errors: {} }));
  }
  ...
  clearError(...args: [...(string[] | [string[]] | []), boolean?]): void {
    let dotsplit = true;
  ...
  getObservableError<K>(field: keyof T) {
    return this.store.pipe(select(state => state?.errors?.[field as string])) as Observable<K>;
  }

  getNestedObservableError<T>(...args: [...(string[] | [string[]] | []), boolean?]): Observable<T> {
    let dotsplit = true;
    ...
    const pathArgs = args as string[] | [string[]];

    const pathArray = BaseStoreRepository.normalizePath(pathArgs, dotsplit);

    return this.store.pipe(
      select((state: any) => {
        return pathArray.reduce((obj, key) => obj?.[key], state?.errors) as T;
      })
    );
  }
